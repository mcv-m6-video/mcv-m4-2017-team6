function [ output_args ] = task_1_2_3( dataset_path, task, conectivity, filling)
    ...
    for dataset=1:3
        if strcmp(task,'hole') %task1
            [ prec(dataset,:),rec(dataset,:),auc(dataset)]=alpha_search( INTERVALS, SEQ(dataset,:), cell2mat(PATH(dataset)),RHO(dataset),cell2mat(SEQ_NAME(dataset)),conectivity, filling, 0,false);
            [ prec_old(dataset,:),rec_old(dataset,:),auc_old(dataset)]=alpha_search( INTERVALS, SEQ(dataset,:), cell2mat(PATH(dataset)),RHO(dataset),cell2mat(SEQ_NAME(dataset)),0, false, 0,false);
        elseif strcmp(task,'opening') %task2
            step=100;
            final=1000;
            for opening=0:step:final
                [ prec(dataset,opening+1,:),rec(dataset,opening+1,:),auc(dataset,opening+1)]=alpha_search( INTERVALS, SEQ(dataset,:), cell2mat(PATH(dataset)),RHO(dataset),cell2mat(SEQ_NAME(dataset)),conectivity, filling, opening,false);
            end
        elseif strcmp(task,'morpho') %task3
            opening=0;
            [ prec(dataset,:),rec(dataset,:),auc(dataset)]=alpha_search( INTERVALS, SEQ(dataset,:), cell2mat(PATH(dataset)),RHO(dataset),cell2mat(SEQ_NAME(dataset)),conectivity, filling, opening,true);
        end
    end
    if strcmp(task,'hole')
        fig=subplot(1,1,1);
